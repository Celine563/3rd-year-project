def findings(
    ...
    if domain_penalties:
        if domain_penalties.get("ssl_penalty", 0) != 0:
            results.append(("SSL Certificate Issue",
                            "Weak or invalid SSL certificates are a red flag."))

        if domain_penalties.get("domain_name_penalty", 0) != 0:
            results.append(("Suspicious Domain Name",
                            "This domain matches known phishing naming patterns."))
